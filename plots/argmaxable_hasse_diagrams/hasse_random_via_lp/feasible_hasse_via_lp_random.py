...
def generate_hasse_diagram_tikz(N, C):
    ...
    # According to paper, uniform(0, 1) is a good approx.
    W = np.random.uniform(0, 1, (N, 2*C + 1))
    # W = np.random.uniform(-1, 1, (N, 2*C + 1))
    ...
    for k in range(N + 1):
        ...
        tikz_code += r'\node[draw, color={}] at ({}, {}) {{Cardinality {}}};'.format('Black', -.9, height, k)
