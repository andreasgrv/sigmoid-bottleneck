...
# from spmlbl.components import KSparseClassifier
...
class VanillaConv(BaseModel):
    ...
    def __init__(self, Y, embed_file, kernel_size, num_filter_maps, clf, gpu=True, dicts=None, embed_size=100, dropout=0.5):
        ...
        #linear output
        # self.fc = nn.Linear(num_filter_maps, Y, bias=False)
        # self.fc = KSparseClassifier(num_filter_maps, Y, k=50)
        # self.fc = KSparseClassifier(num_filter_maps, Y, k=50, slack_dims=99)
        # NOTE: DxD matrix invalidates init.
        # self.fc = KSparseClassifier(num_filter_maps, Y, k=50, slack_dims=99, freeze=False, use_init=True, use_reparam=False)
        # self.fc = KSparseClassifierV(num_filter_maps, Y, k=50, slack_dims=99, use_init=False)
        # TODO:
        # self.fc = KSparseClassifier(num_filter_maps, Y, k=50, use_init=False, use_reparam=True)
