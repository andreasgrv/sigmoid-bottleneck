...
def train_epochs(args, model, optimizer, params, dicts):
    ...
    for epoch in range(args.n_epochs):
        ...
        if epoch == 0 and not args.test_model:
            model_dir = os.path.join(MODEL_DIR, '_'.join([args.model, args.clf, 'k%d' % args.k,
                                                          's%d' % args.slack_dims,
                                                          'seed%d' % args.seed]))
            os.mkdir(model_dir)
        ...
        metrics_hist_tr['time_per_epoch'].append(time_per_epoch)
...
if __name__ == "__main__":
    ...
    # 2023-08-15 00:42 Added by X
    parser.add_argument("--clf", type=str, choices=["bottleneck", "vander", "fft", "grass"], help="Which classifier head to use", default="bottleneck")
    parser.add_argument("--k", type=int, default=100, help='The max number of active labels we expect (dim = 2k+1).')
    parser.add_argument("--slack-dims", type=int, default=0, help='Slack dim for MLP')
    # -----------------------------
