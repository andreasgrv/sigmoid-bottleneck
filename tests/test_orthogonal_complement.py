...
def test_cyclic_position_example(plot=False):

    np.random.seed(13)
    N, D = 6, 2

    # This is already affinised (D=3)
    W = vander(N, D+1) * np.sqrt(N)
    print(W)
    N, D = W.shape
    ...
    # If W is in general position (cyclic is gen position), the relative interior of an orthant is either:
    ...
    print('The circuits of W are (Notice the alternating signs!):')
...
if __name__ == "__main__":
    ...
    test_cyclic_position_example(plot=True)
